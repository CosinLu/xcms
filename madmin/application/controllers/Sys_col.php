...
class Sys_col extends MY_Controller
{
    ...
    {
        $url['get_list_url'] = site_url('sys_col/get_list');
        $url['insert_url'] = '<a href="' . site_url('sys_col/insert') . '" class="btn btn-default">新增</a>';
        $url['save_url'] = site_url('sys_col/save');
        $url['del_url'] = site_url('sys_col/del');
        $this->load->vars($url);
    }
    ...
    public function get_list()
    {
        ...
        foreach ($data['list']['list'] as $key => $val) {
            ...
            $data['list']['list'][$key]['insert_next_btn'] = '<a href="' . site_url('sys_col/insert?sys_cid=' . $this->sys_cid . '&id=' . $val['id']) . '">新增属性</a>';
