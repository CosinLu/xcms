...
class Sys_dict extends MY_Controller
{
    ...
    public function set_url()
    {
        ...
        $url['insert_btn'] = $this->sys_auth->set_auth(MYINSERT, $this->col_auth, '<a class="btn btn-primary btn-sm" href="' . site_url('sys_dict/insert?sys_cid=' . $this->sys_cid) . '">新增</a>');
    ...
    public function get_list()
    {
        ...
        foreach ($data['list']['list'] as $key => $val) {
            ...
            if ($val['level'] == 1) {
                $data['list']['list'][$key]['opera_btn'][] = $this->sys_auth->set_auth(MYINSERT, $this->col_auth, '<a href="' . site_url('sys_dict/insert?sys_cid=' . $this->sys_cid . '&id=' . $val['id']) . '">新增属性</a>', '<a href="javascript:;" class="disabled">新增属性</a>');
            }
            $data['list']['list'][$key]['opera_btn'][] = $this->sys_auth->set_auth(MYUPDATE, $this->col_auth, '<a href="' . site_url('sys_dict/update?sys_cid=' . $this->sys_cid . '&id=' . $val['id']) . '">编辑</a>', '<a href="javascript:;" class="disabled">编辑</a>');
            $data['list']['list'][$key]['opera_btn'][] = $this->sys_auth->set_auth(MYDEL, $this->col_auth, '<a href="javascript:;" data-name="delCol" data-id="' . $val['id'] . '">删除</a>', '<a href="javascript:;" class="disabled">删除</a>');
    ...
    public function insert()
    {
        ...
        $data['cols'] = $this->category->ddl('pid', 0, $id, $data['sys_dict_type']);
    ...
    public function update()
    {
        ...
        $data['sys_dict_type'] = $this->sys_dict->sys_dict_type();
        $data['cols'] = $this->category->ddl('pid', $id, $data['item']['pid'], $data['sys_dict_type']);
