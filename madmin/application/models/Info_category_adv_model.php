 ...
 * Time: 21:12
 */
class info_category_adv_model extends CI_Model
{
    ...
    {
        $this->db->where('id', $id);
        $res = $this->db->get('info_category_adv')->row_array();
    ...
    {
        if ($post['id']) {
            $bool = $this->db->where('id', $post['id'])->update('info_category_adv', $post['vals']);
        } else {
            $bool = $this->db->insert('info_category_adv', $post['vals']);
        }
